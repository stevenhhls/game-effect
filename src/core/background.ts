export function validateBackgroundFile(file: Pick<File, 'name' | 'type' | 'size'>): void {
  const supported = file.type ? ['image/png', 'image/jpeg', 'image/webp'].includes(file.type) : /\.(png|jpe?g|webp)$/i.test(file.name);
  if (!supported) throw new Error('Choose a PNG, JPG or WebP image.');
  if (file.size === 0) throw new Error('The selected image is empty.');
  if (file.size > 20 * 1024 * 1024) throw new Error('Choose an image smaller than 20 MB.');
}

export function backgroundSize(width: number, height: number, textureLimit: number): {width: number; height: number} {
