...
export function setupPanelResize() {
    ...
    const library = main.querySelector('.library');
    ...
    try {
        ...
        if (Array.isArray(saved) && saved.length === 2 && saved.every(x => typeof x === 'number' && Number.isFinite(x)))
            widths = saved;
    }
    catch { }
    ...
    const handles = [0, 1].map(index => {
        ...
        (index === 0 ? library : stage).after(handle);
        return handle;
    });
    function apply() {
        if (!wide.matches)
    ...
    handles.forEach((handle, index) => {
        ...
        handle.onpointerdown = event => {
            ...
            drag = { id: event.pointerId, x: event.clientX, width: widths[index] };
        ...
        handle.onpointermove = event => {
            ...
            widths[index] = Math.max(index === 0 ? 130 : 240, Math.min(requested, available() - widths[1 - index] - 320));
            apply();
        };
        const finish = () => { if (!drag)
            return; drag = undefined; document.body.classList.remove('resizing-panels'); save(); };
        ...
        handle.onkeydown = event => {
            if (event.key !== 'ArrowLeft' && event.key !== 'ArrowRight')
                return;
            event.preventDefault();
            const delta = (event.key === 'ArrowRight' ? 1 : -1) * (index === 0 ? 1 : -1) * (event.shiftKey ? 40 : 10);
            widths[index] = Math.max(index === 0 ? 130 : 240, Math.min(widths[index] + delta, available() - widths[1 - index] - 320));
            apply();
            save();
        };
    });
