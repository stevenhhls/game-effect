...
if (!context)
    throw new Error('WebGL unavailable. Enable hardware acceleration.');
const gl = context;
const memory = {};
const comparisonKey = 'fx-studio-alien-study-v1';
let comparisonSettings = defaults(alienDetailed);
try {
    comparisonSettings = mergeComparison(alienDetailed, {}, JSON.parse(localStorage.getItem(comparisonKey) || '{}'));
}
catch { }
for (const effect of effects) {
    let saved = {};
    try {
        saved = JSON.parse(localStorage.getItem((effect.id === 'alien' ? 'fx-studio-v4-' : 'fx-studio-v1-') + effect.id) || '{}');
    }
    catch { }
...
let locations = {};
let texture;
function report(error) { el('error').textContent = String(error); }
function compile(type, source) {
    ...
    if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
        ...
        throw Error(msg || 'Shader compile failed');
    }
    return shader;
}
function program(fragment) {
    ...
    if (!gl.getProgramParameter(p, gl.LINK_STATUS))
        throw Error(gl.getProgramInfoLog(p) || 'Link failed');
    return p;
}
...
function sync() {
    ...
    if (isAlienStudy(active)) {
        ...
        Object.assign(memory, comparisonVariants(alienStudies, comparisonSettings));
        try {
            localStorage.setItem(comparisonKey, JSON.stringify(comparisonSettings));
        }
        catch { }
    }
    else {
        try {
            localStorage.setItem((active.id === 'alien' ? 'fx-studio-v4-' : 'fx-studio-v1-') + active.id, JSON.stringify(settings));
        }
        catch { }
    }
}
function replay() { elapsed = 0; start = performance.now(); running = true; el('pause').textContent = 'Pause'; }
function controls() {
    ...
    for (const p of active.parameters) {
        ...
        range.type = 'range';
        range.setAttribute('aria-label', p.label + ' slider');
        for (const field of [number, range]) {
            field.min = String(p.min);
            field.max = String(p.max);
            field.step = String(p.step);
            field.value = String(settings[p.key]);
        }
        label.append(number);
        const help = document.createElement('small');
        help.textContent = p.help;
        row.append(label, help, range);
        root.append(row);
        const change = (field) => {
            if (!Number.isFinite(field.valueAsNumber))
                return;
            settings = normalize(active, { ...settings, [p.key]: field.valueAsNumber });
            number.value = range.value = String(settings[p.key]);
            sync();
            replay();
        };
...
function select(effect) {
    ...
    hasSelection = true;
    active = effect;
    settings = { ...memory[effect.id] };
    history.replaceState(null, '', '#' + effect.id);
    if (ready) {
        current = programs.get(effect.id);
        locations = Object.fromEntries(Object.keys(frameUniforms(effect, settings, 0, 1)).map(k => [k, gl.getUniformLocation(current, 'u_' + k)]));
    }
    el('title').textContent = effect.name;
    el('description').textContent = effect.description;
    document.querySelectorAll('[data-effect]').forEach(b => b.setAttribute('aria-pressed', String(b.dataset.effect === effect.id)));
...
for (const effect of effects) {
    ...
    small.textContent = effect.category;
    button.append(small);
    button.onclick = () => { if (ready)
        select(effect); };
    el('effects').append(button);
}
for (const button of document.querySelectorAll('#comparison [data-effect]')) {
    button.onclick = () => { const effect = effects.find(e => e.id === button.dataset.effect); if (ready && effect)
        select(effect); };
}
window.addEventListener('hashchange', () => {
    const effect = effects.find(e => e.id === location.hash.slice(1));
    if (ready && effect && effect !== active)
        select(effect);
});
el('reset').onclick = () => {
    if (isAlienStudy(active)) {
        comparisonSettings = defaults(alienDetailed);
        Object.assign(memory, comparisonVariants(alienStudies, comparisonSettings));
    }
...
canvas.onclick = event => {
    ...
    settings.centerX = Number(((event.clientX - rect.left) / rect.width).toFixed(3));
    settings.centerY = Number(((event.clientY - rect.top) / rect.height).toFixed(3));
    controls();
    replay();
};
function frame(now) {
    if (running) {
        elapsed = now - start;
        if (elapsed > settings.duration + 500) {
    ...
    for (const [key, value] of Object.entries(frameUniforms(active, settings, progress, canvas.width / canvas.height)))
        gl.uniform1f(locations[key], value);
    gl.drawArrays(gl.TRIANGLES, 0, 6);
    if (input('compare').checked) {
        gl.enable(gl.SCISSOR_TEST);
        gl.scissor(0, 0, Math.floor(canvas.width / 2), canvas.height);
        gl.useProgram(plain);
        gl.drawArrays(gl.TRIANGLES, 0, 6);
        gl.disable(gl.SCISSOR_TEST);
    }
    input('scrub').value = String(progress);
    el('time').textContent = Math.round(progress * settings.duration) + ' / ' + settings.duration + ' ms';
    requestAnimationFrame(frame);
}
async function boot() {
    try {
        for (const effect of effects)
            programs.set(effect.id, program(effect.fragment));
        ...
        report(error);
        el('status').textContent = 'Preview unavailable';
    }
}
void boot();
